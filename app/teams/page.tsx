...
export default function TeamsPage() {
  ...
  const filteredTeams = teams.filter((team) => {
    ...
    const matchesEvent = selectedEventFilter === "all" || team.event_id.toString() === selectedEventFilter
      ...
      <div className="min-h-screen bg-gradient-to-b from-[#131943] to-[#1a1f4a] flex items-center justify-center">
        <Card className="max-w-md w-full bg-[#556492]/20 border-[#556492]/30">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-[#D4D4D6]">Please sign in to view and manage teams</CardDescription>
          </CardHeader>
          <CardContent>
            <Link href="/sign-in">
              <Button className="w-full bg-[#84C25D] hover:bg-[#6ba348] text-white">Sign In</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="container px-4 py-12 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-12">
          <div className="space-y-4">
            <h1 className="text-4xl font-bold tracking-tighter text-white sm:text-5xl md:text-6xl">Team Management</h1>
            <p className="mx-auto max-w-[700px] text-[#D4D4D6] md:text-xl">
              Create teams, join existing ones, and collaborate with fellow participants for team-based events
            </p>
            <div className="flex flex-wrap gap-2 justify-center">
              <Badge className="bg-[#84C25D] text-white">
                <Users className="h-3 w-3 mr-1" />
                {teams.length} Teams
              </Badge>
              <Badge className="bg-[#7683A4] text-white">Team Events Available</Badge>
            </div>
          </div>
        </div>
              ...
              <DialogContent className="bg-[#131943] border-[#556492]">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-[#D4D4D6]">
                    Create a team for team-based events and invite members to join
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    <Button onClick={handleCreateTeam} className="flex-1 bg-[#84C25D] hover:bg-[#6ba348] text-white">
                      Create Team
                    </Button>
                  </div>
                </div>
        ...
        {myTeams.length > 0 && (
          <div className="mb-12">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {myTeams.map((team) => (
                <Card key={team.id} className="bg-gradient-to-br from-[#84C25D]/20 to-[#6ba348]/20 border-[#84C25D]/30">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-white flex items-center gap-2">
                        <Crown className="h-5 w-5 text-[#84C25D]" />
                        {team.name}
                      </CardTitle>
                      <Badge className="bg-[#84C25D] text-white">Leader</Badge>
                    </div>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="text-sm">
                        ...
                        <span className="text-white ml-2">{new Date(team.created_at).toLocaleDateString()}</span>
                      </div>
                      <div className="space-y-2">
                        <span className="text-sm text-[#D4D4D6]">Members:</span>
                        {team.team_members.map((member) => (
                          <div key={member.id} className="flex items-center gap-2 text-sm">
                            <Users className="h-3 w-3 text-[#84C25D]" />
                            <span className="text-white">{member.users.full_name}</span>
                            {member.role === "leader" && <Crown className="h-3 w-3 text-[#84C25D]" />}
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {filteredTeams.length === 0 ? (
            <Card className="bg-[#556492]/20 border-[#556492]/30">
              <CardContent className="flex flex-col items-center justify-center py-12">
                <Users className="h-12 w-12 text-[#84C25D] mb-4" />
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredTeams.map((team) => (
                <Card
                  key={team.id}
                  className="bg-gradient-to-br from-[#556492]/10 to-[#7683A4]/10 border-[#556492]/30 hover:border-[#84C25D]/50 transition-all duration-300"
                >
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-white">{team.name}</CardTitle>
                      <Badge variant="outline" className="border-[#7683A4] text-[#7683A4]">
                        {team.events.event_type}
                      </Badge>
                    </div>
                    <CardDescription className="text-[#D4D4D6]">{team.events.name}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-[#D4D4D6]">Members:</span>
                        <span className="text-white">{team.team_members.length}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-[#D4D4D6]">Created:</span>
                      ...
                      <div className="space-y-1">
                        {team.team_members.slice(0, 3).map((member) => (
                          <div key={member.id} className="flex items-center gap-2 text-xs">
                            <Users className="h-3 w-3 text-[#84C25D]" />
                            <span className="text-[#D4D4D6]">{member.users.full_name}</span>
                            {member.role === "leader" && <Crown className="h-3 w-3 text-[#84C25D]" />}
                          </div>
                        ))}
                        {team.team_members.length > 3 && (
                          <div className="text-xs text-[#D4D4D6]">+{team.team_members.length - 3} more members</div>
                        )}
                      </div>
                    </div>
                  </CardContent>
