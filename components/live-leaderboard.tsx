...
export function LiveLeaderboard() {
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default:
        return <span className="text-[#84C25D] font-bold">#{rank}</span>
    }
  }
      ...
      <Card className="bg-[#556492]/20 border-[#556492]/30">
        <CardContent className="flex items-center justify-center py-12">
          <motion.div
            ...
            className="w-8 h-8 border-4 border-[#84C25D] border-t-transparent rounded-full"
          />
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          <TrendingUp className="h-6 w-6 text-[#84C25D]" />
          Live Leaderboard
        </h2>
              ...
              <CardContent className="space-y-3">
                {globalLeaderboard.length === 0 ? (
                  <div className="text-center py-8">
                    <Users className="h-12 w-12 text-[#84C25D] mx-auto mb-4" />
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <Avatar className="h-10 w-10">
                            <AvatarImage src={entry.users.profile_picture || ""} />
                            <AvatarFallback className="bg-[#84C25D] text-white">
                              {entry.users.full_name.charAt(0)}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                            <h3 className="font-semibold text-white">{entry.users.full_name}</h3>
                            <p className="text-sm text-[#D4D4D6]">{entry.users.institution}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-lg font-bold text-[#84C25D]">{entry.total_points} pts</div>
                          <div className="text-sm text-[#D4D4D6]">
                            {entry.events_participated} events • {entry.events_won} wins
                          </div>
                        </div>
                      </div>
                    </motion.div>
            ...
            {eventLeaderboards.length === 0 ? (
              <Card className="bg-[#556492]/20 border-[#556492]/30">
                <CardContent className="text-center py-8">
                  ...
                  <p className="text-[#D4D4D6]">No event rankings available yet</p>
                </CardContent>
              </Card>
            ) : (
              eventLeaderboards.map((eventBoard) => (
                ...
                >
                  <CardHeader>
                    <CardTitle className="text-white">{eventBoard.event_name}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {eventBoard.participants.slice(0, 5).map((participant, index) => (
                      <div
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="font-medium text-white">{participant.users.full_name}</h4>
                            <p className="text-xs text-[#D4D4D6]">{participant.users.institution}</p>
                          </div>
                        </div>
                        <Badge className="bg-[#84C25D] text-white">{participant.points} pts</Badge>
                      </div>
                    ))}
                  </CardContent>
                </Card>
