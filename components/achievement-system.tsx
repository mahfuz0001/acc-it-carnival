...
export function AchievementSystem() {
  ...
  const checkForNewAchievements = async () => {
    ...
    const newlyUnlocked = achievements.filter(
      (achievement) => !achievement.is_unlocked && achievement.current_progress >= achievement.requirement,
    )
  ...
  const categories = [
    ...
    { id: "participation", label: "Participation", icon: <Calendar className="h-4 w-4" /> },
  ...
  return (
    <div className="space-y-6">
      {/* User Stats Overview */}
      {userStats && (
        <Card className="bg-gradient-to-r from-[#84C25D]/20 to-[#6ba348]/20 border-[#84C25D]/30">
          <CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-[#84C25D]">{userStats.total_points}</div>
                <div className="text-sm text-[#D4D4D6]">Total Points</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-[#84C25D]">#{userStats.rank}</div>
                <div className="text-sm text-[#D4D4D6]">Global Rank</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-[#84C25D]">{userStats.achievements_unlocked}</div>
                <div className="text-sm text-[#D4D4D6]">Achievements</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-[#84C25D]">{userStats.events_completed}</div>
          ...
          {filteredAchievements.map((achievement) => (
            ...
            >
              ...
              >
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Badge className="bg-[#84C25D] text-white text-xs">{achievement.points} pts</Badge>
                    </div>
                  </div>
                ...
                <CardContent>
                  <p className="text-[#D4D4D6] text-sm mb-3">{achievement.description}</p>

                  {!achievement.is_unlocked && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-[#D4D4D6]">Progress</span>
                        <span className="text-[#84C25D]">
                  ...
                  {achievement.is_unlocked && achievement.unlocked_at && (
                    <div className="flex items-center gap-2 text-sm text-[#84C25D]">
      ...
      <AnimatePresence>
        {newAchievements.map((achievement) => (
          ...
          >
            <Card className="bg-gradient-to-br from-[#84C25D]/20 to-[#6ba348]/20 border-[#84C25D] max-w-md mx-4">
              <CardContent className="text-center p-6">
                ...
                <h4 className="text-xl text-[#84C25D] mb-2">{achievement.title}</h4>
                <p className="text-[#D4D4D6] mb-4">{achievement.description}</p>
                <Badge className="bg-[#84C25D] text-white">+{achievement.points} points</Badge>
              </CardContent>
            </Card>
          </motion.div>
        ))}
