...
import { Play, Pause, Volume2, VolumeX, Maximize, Users, MessageCircle } from "lucide-react"
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setStreams((prev) =>
        prev.map((stream) => ({
          ...stream,
          viewer_count: stream.viewer_count + Math.floor(Math.random() * 10) - 5,
        })),
      )
    }, 5000)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          <Play className="h-6 w-6 text-[#84C25D]" />
          Live Streams
        </h2>
        <Badge className="bg-red-500 text-white animate-pulse">{liveStreams.length} LIVE</Badge>
      </div>
      ...
      {activeStream && (
        <Card className="bg-gradient-to-br from-[#556492]/10 to-[#7683A4]/10 border-[#556492]/30 overflow-hidden">
          <CardContent className="p-0">
            <div className="relative aspect-video bg-black">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-gray-900 to-black flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-400">{activeStream.stream_url}</p>
                </div>
              </div>
            ...
            {/* Stream Info */}
            <div className="p-4">
              <h3 className="text-xl font-bold text-white mb-2">{activeStream.title}</h3>
              ...
              <div className="flex items-center gap-4">
                ...
                {activeStream.is_live && <Badge className="bg-red-500 text-white">Live Now</Badge>}
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        {liveStreams.length > 0 && (
          <Card className="bg-gradient-to-br from-red-500/10 to-red-600/10 border-red-500/30">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">🔴 Live Now</CardTitle>
            </CardHeader>
                ...
                <motion.div
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${
                    activeStream?.id === stream.id
                      ? "bg-[#84C25D]/20 border-[#84C25D]"
                      : "bg-[#556492]/20 border-[#556492]/30 hover:border-[#84C25D]/50"
                  }`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-[#D4D4D6]">{stream.viewer_count.toLocaleString()} viewers</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
        ...
        {recordedStreams.length > 0 && (
          <Card className="bg-gradient-to-br from-[#556492]/10 to-[#7683A4]/10 border-[#556492]/30">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">📹 Recordings</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-white truncate">{stream.title}</h4>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge className="bg-gray-500 text-white text-xs">RECORDED</Badge>
                        <span className="text-xs text-[#D4D4D6]">{stream.viewer_count.toLocaleString()} views</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
