...
export function RealTimeNotifications() {
  ...
  const getNotificationColor = (type: string) => {
    switch (type) {
      ...
      case "registration_confirmed":
        return "bg-[#84C25D]/20 border-[#84C25D]/30";
      case "submission_received":
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative text-[#D4D4D6] hover:text-[#84C25D]"
        >
          <Bell className="h-5 w-5" />
          {unreadCount > 0 && (
            <Badge className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-[#84C25D] text-white text-xs flex items-center justify-center p-0">
          ...
          <div className="flex items-center justify-between">
            ...
            {unreadCount > 0 && (
              <Button
                ...
                className="text-[#84C25D] hover:text-[#6ba348] text-xs"
              >
                Mark all read
              </Button>
            )}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          {!notification.is_read && (
                            <div className="w-2 h-2 bg-[#84C25D] rounded-full flex-shrink-0 ml-2" />
                          )}
                        </div>
